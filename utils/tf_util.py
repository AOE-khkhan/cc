...
def _weight_variable(shape, regularization=True, name='Weight'):
    regularizers = []
...
def chebyshev(x, L, Fout, K, name):
    # If K == 1 it is equivalent to fc layer
    Batch_size, Num_points, N, F = x.get_shape()
    Batch_size, Num_points, N, F = int(Batch_size), int(Num_points), int(N), int(F)
    x0 = x  # N x M x Fin
    ...
    x = tf.reshape(x, [Batch_size * Num_points * N, F * K])  # N*M x Fin*K
    # Filter: Fin*Fout filters of order K, i.e. one filterbank per feature pair.
    W = _weight_variable([F * K, Fout], regularization=False, name=name)
    x = tf.matmul(x, W)  # N*M x Fout
    return tf.reshape(x, [Batch_size, Num_points, N, Fout])  # N x M x Fout


def ngcn_chebyshev(inputs,
                num_output_channels,
                scope,
                nn_k=None,
                local_cord=None,
    ...
    outputs = chebyshev(inputs, L, num_output_channels, K=3, name=scope)
...
def pointSIFT_KNN(radius, xyz):
    ...
    xyz_central = tf.tile(tf.expand_dims(xyz, 2), [1, 1, 32, 1])  # translation normalization
...
def group(xyz, idx):
    ...
    xyz_central = tf.tile(tf.expand_dims(xyz, 2), [1, 1, 32, 1])
    a = grouped_xyz - xyz_central
    new_points = tf.concat([xyz_central, a], axis=-1)
