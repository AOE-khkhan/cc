...
parser.add_argument('--decay_step', type=int, default=250000, help='Decay step for lr decay [default: 200000]')
...
LOG_DIR_prefix = 'log'
LOG_DIR = os.path.join(BASE_DIR,LOG_DIR_prefix)
if not os.path.exists(LOG_DIR):
    new_log_dir = LOG_DIR + '/' + str(1)
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            acc,miou = eval_one_epoch(sess, ops, test_writer,epoch)

            if epoch > 1 and (miou > best_iou):
                best_iou = miou
                save_path = saver.save(sess, os.path.join(LOG_DIR, "best_model_epoch_%03d.ckpt"%(epoch)))
                log_string("Model saved in file: %s" % save_path)
                log_string_record('**** EPOCH %03d ****' % (epoch) , LOG_FOUT_max_record)
                log_string_record('eval accuracy: %f'% acc , LOG_FOUT_max_record)
                log_string_record('eval mIOU: %f'% miou , LOG_FOUT_max_record)
...
def eval_one_epoch(sess, ops, test_writer,epoch):
    ...
    summary_acc = tf.Summary(value=[
                                    tf.Summary.Value(
                                                     tag='eval_acc', simple_value=float((total_correct / float(total_seen)))),
                                    tf.Summary.Value(
                                                     tag='eval_mIOU_all_shape', simple_value=float((np.mean(all_shape_ious)))),
                                    tf.Summary.Value(
                                                     tag='eval_mIOU', simple_value=float(mean_shape_ious))
                                    ])
